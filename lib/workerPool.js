...
class WorkerPool {
    constructor(WorkerConstructor, poolSize = Math.max(navigator.hardwareConcurrency / 2, 4)) {
    ...
    _processQueue() {
        if (this.taskQueue.length > 0 && this.idleWorkers.length > 0) {
            const { method, resolve, reject, type, args = [], worker = this.idleWorkers.shift()} = this.taskQueue.shift();
            if (type === 'request') {
    ...
    addTaskToWorker(worker, method, args = [], type = 'request') {
        return this.addTaskComplex(method, args, type, worker);
    }
    ...
    addTaskComplex(method, args = [], type = 'request', worker) {
        if (this._terminated) {
        ...
        return new Promise((resolve, reject) => {
            this.taskQueue.push({ method, resolve, reject, type, args, worker });
            this._processQueue();
        });
    }
    ...
    addRequestTaskToWorker(worker, method, args = []) {
        return this.addTaskComplex(method, args, 'request', worker);
    }
    ...
    addNotifyTaskToWorker(worker, method, args = []) {
        return this.addTaskComplex(method, args, 'notify', worker);
    }
